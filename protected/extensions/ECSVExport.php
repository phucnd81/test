...
class ECSVExport
{
     ...
     */    
    public function __construct($dataProvider = array(), $exportFull=true, $includeColumnHeaders=true, $delimiter=null, $enclosure=null) 
    {
    ...
    protected function _writeHeaders($row)
    {
        ...
        $this->fputcsv_new($this->_filePointer, $headers);
    }
    ...
    public function _writeRow($row)
    {
     ...
     //   array_walk($row,array('ECSVExport','nhamnhi'));       
        if(isset($this->_callback) && $this->_callback) {
          //  fputcsv($this->_filePointer, call_user_func($this->_callback, $row), $this->_delimiter, $this->_enclosure);   
            $this->fputcsv_new($this->_filePointer, call_user_func($this->_callback, $row));                    
        } else {
           // fputcsv($this->_filePointer, $row, $this->_delimiter, $this->_enclosure);
            $this->fputcsv_new($this->_filePointer, $row);
        }
        unset($row);
    }
    ...
    function fputcsv_new ($fp, $array, $deliminator=",") { 
	...
	public function fputcsv_new_line ($array, $deliminator=",") { 
	...
	public function remove_new_lines($val, $new_lines = false, $deliminator=","){
		 # remove any windows new lines, 
        # as they interfere with the parsing at the other end 
        $val = str_replace("\r\n", "\n", $val); 
    	if ( !$new_lines )
