...
class CDbCriteria extends CComponent
{
	...
	public function addLikeCondition($column, $keyword, $format = '%{value}%', $escape=true, $operator='AND'){
		if($keyword=='')
			return $this;
		...
		$keyword = strtr($format, array('{value}'=>$keyword));
	...
	public function addBetween($column,$valueStart,$valueEnd,$operator='AND'){
		if(!empty($valueStart) && !empty($valueEnd))
			return $this->addBetweenCondition($column,$valueStart,$valueEnd);
		else if(!empty($valueStart)){
			$paramStart=self::PARAM_PREFIX.self::$paramCount++;
			$this->params[$paramStart]=$valueStart;
			$condition="$column >= $paramStart";
			return $this->addCondition($condition, $operator);
		}
		else if(!empty($valueEnd)){
			$paramEnd=self::PARAM_PREFIX.self::$paramCount++;
			$this->params[$paramEnd]=$valueEnd;
			$condition="$column <= $paramEnd";
			return $this->addCondition($condition, $operator);
		}		
	}
