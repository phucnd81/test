...
class CDbCommand extends CComponent
{
	...
	{
		if(empty($query) && !empty($this->_criteria) ){
			$array = $this->_criteria->toArray();
			if(!empty($this->_query['from'])){
				$array['from'] = $this->_query['from'];
			}
				
			if(isset($array['condition'])){
				$array['where'] = $array['condition'];
			}	
	...
	public function updateCriteria($query = array()){
	...
	public function count($select = 'COUNT(*)'){
		$array = array();
		if(!empty($this->_criteria)){
			$array = $this->_criteria->toArray();
		}
		...
		$this->setText($this->buildQuery($array));
		return $this->queryScalar($array['params']);
	}	
