 ...
 *
 * Depends:
 *  jquery.ui.jpdatepicker.js
 *  jquery.ui.jpdatepicker-(region).js
 *  jquery.global.js
    ...
    $.fn.extend($.jpdatepicker, {
        ...
        _attachments: function (input, inst) {
            ...
            if (syncAltField == true) {
                ...
                inst.altField.change(function () {
                    input.val(eval('$.jpdatepicker.formatDate("' + inst.settings.dateFormat + '", $.jpdatepicker.parseDisplayingDate($(this).val()), $.jpdatepicker.settings)'));
                });
                if (input.val()) {
                    try {
                ...
                for (var col = 0; col < numMonths[1]; col++) {
                    ...
                    for (var dRow = 0; dRow < numRows; dRow++) { // create date picker rows
                        ...
                        for (var dow = 0; dow < 7; dow++) { // create date picker days
							...
							(unselectable ? '' : ' onclick="DP_jQuery_' + dpuuid + '.jpdatepicker._selectDay(\'#' +
        ...
        _selectDay: function (id, month, year, td) {
            ...
            if (inst.altField) {
				...
				this._selectEra(id, year, inst.currentMonth, inst.currentDay);
            }
        },
		
		_selectEra: function(id, year, month, day){
			toJapaneseYearString = function(year, type){
				if(!year) return;
					year = parseInt(year);
				...
				var prefix = type === true ? 'short_name' : 'long_name';
				if(year < wareki[0].start) return year;
				if(year < wareki[1].start) return wareki[0][prefix]; 
				if(year < wareki[2].start) return wareki[1][prefix]; 
				if(year < wareki[3].start) return wareki[2][prefix];
				if(year == 1989 && month == 0 && day < 8) return wareki[2][prefix];
				
				return wareki[3][prefix];
			};
			
			if($(id + '_era').length>0){
				if(era = toJapaneseYearString(year, true)){
					$(id + '_era').find('option').each(function(){
						this.selected = false;	
						if(era == $(this).val()){
							this.selected = true;	
						}
